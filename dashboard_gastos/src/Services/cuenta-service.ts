...
import { CuentaMesSchema, CuentaSemanaSchema } from '@/Schemas/cuentaSchema'
...
export async function getMes() {
    ...
    const url = `${import.meta.env.VITE_API_URL}cuentas/calcular-mes`

    try {
        const {data: response} = await axios.get(url);
        // console.log(response)

        const result = CuentaMesSchema.safeParse(response)
        // console.log(result)
