...
export const Card = ({ data, type}: cardProps) => {
  ...
  const classCard = `bg-white rounded-xl shadow p-3 md:p-5 flex md:flex-row flex-col items-center justify-between overflow-hidden h-auto md:h-[6.875rem]}`;
  ...
  const iconClass = `text-[1.125rem] md:text-[1.25rem] ${data.colorText}`;
    ...
    <div className={type === 'card' ?  classCard : classCategory } >
      ...
      :(<>
        ...
        <div className={`${type === 'card' ? "md:text-left " : "" } text-center flex-grow`}>
          <div className={`${type === 'card' ? "md:justify-end md:text-2xl  " : ""} flex justify-center font-medium text-[0.875rem]  break-words overflow-wrap-break-word`}>
            ${formatMoneda(data.amount)}
          </div>
          <div className={`${type === 'card' ? "md:justify-end md:text-sm mt-1  font-semibold" : ""} flex justify-center font-medium  text-gray-600 text-[0.75rem] break-words overflow-wrap-break-word`}>
            {data.name}
          </div>
        </div>
      </>)}
